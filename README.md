...
observe in the Table below, the Flair library results in better sentiment analysis accuracy. Nevertheless, the runtime of Flair
is a major problem since it does not scale currently for a large dataset. One of our priorities will be to tackle this issue by
parallelizing the function that Flair uses so that we could run it over multiple CPUs with a partitioned dataset at the same time.
